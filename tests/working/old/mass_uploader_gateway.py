...
# --- REWRITTEN FUNCTION WITH MINIMAL PAYLOAD CONSTRUCTION ---
def recreate_assets_with_new_firmware(client: IcareApiClient, server_data: List[Dict]):
    """
    Performs a deep recreation of Transmitters (and Channels) and a simple
    recreation of Gateways to update firmware, using minimal payloads.
    """
    print("\n--- Starting Minimal Payload Firmware Update Process ---")
    ...
    for asset_summary in assets_to_recreate:
        ...
        # --- Logic for Transmitters (Deep Recreation) ---
        if asset_type == "Transmitter":
            newly_created_transmitter_id = None
            try:
                # 1. Get full info for transmitter and its channels
                print("  [1/6] Getting full transmitter and channel info...")
                old_transmitter_full = client.get_asset(asset_id)
                stored_channel_payloads = []
                for channel in old_transmitter_full.get('channels', []):
                    # Construct minimal payload for channel recreation
                    channel_payload = {
                        't': channel.get('t'),
                        'name': channel.get('name'),
                        'path': channel.get('path'),
                        'channel': channel.get('channel'),
                        'sensortype': channel.get('sensortype')
                    }
                    stored_channel_payloads.append(channel_payload)
                print(f"  -> Stored {len(stored_channel_payloads)} channel(s) for recreation.")

                # 2. Delete the channels
                print("  [2/6] Deleting old channels...")
                for channel in old_transmitter_full.get('channels', []):
                    client.delete_asset(channel['_id'], channel['_etag'])
                print("  -> Old channels deleted.")

                # 3 & 4. Delete the old transmitter
                print("  [3,4/6] Deleting old transmitter...")
                etag_for_delete = client.get_asset(asset_id).get('_etag')
                client.delete_asset(asset_id, etag_for_delete)
                print("  -> Old transmitter deleted.")

                # 5. Construct minimal payload and create new transmitter
                print("  [5/6] Creating new transmitter with updated firmware...")
                optionals_payload = old_transmitter_full.get('optionals', {}).copy()
                optionals_payload['appfirmware'] = FIRMWARE_MAP[33554435]

                new_transmitter_payload = {
                    't': old_transmitter_full.get('t'),
                    'name': old_transmitter_full.get('name'),
                    'path': old_transmitter_full.get('path'),
                    'serialnumber': old_transmitter_full.get('serialnumber'),
                    'mac': old_transmitter_full.get('mac'),
                    'optionals': optionals_payload
                }
                
                created_transmitter = client.create_asset(new_transmitter_payload)
                newly_created_transmitter_id = created_transmitter.get('_id')
                print(f"  -> New transmitter created with ID: {newly_created_transmitter_id}")

                # 6. Recreate the channels
                print("  [6/6] Recreating channels...")
                for channel_payload in stored_channel_payloads:
                    channel_payload['path'][-1] = newly_created_transmitter_id # Link to new transmitter
                    client.create_asset(channel_payload)
                print("  -> Channels recreated successfully.")
                print(f"  ✅ Deep recreation for '{asset_name}' complete.")

            except Exception as e:
                print(f"  ❌ ERROR: Critical error during deep recreation of '{asset_name}'.")
                print(f"    Details: {e}")
                print("    !! ATTENTION: Manual verification required.")

        # --- Logic for Gateways (Simple Recreation) ---
        else:
            try:
                old_gateway_full = client.get_asset(asset_id)
                old_gateway_etag = old_gateway_full.get('_etag')

                # Construct minimal payload for new gateway
                optionals_payload = old_gateway_full.get('optionals', {}).copy()
                optionals_payload['appfirmware'] = FIRMWARE_MAP[33554433]

                new_gateway_payload = {
                    't': old_gateway_full.get('t'),
                    'name': old_gateway_full.get('name'),
                    'path': old_gateway_full.get('path'),
                    'unique_id': old_gateway_full.get('unique_id'),
                    'optionals': optionals_payload
                }

                print("  [1/2] Creating new gateway with updated firmware...")
                created_gateway = client.create_asset(new_gateway_payload)
                
                print("  [2/2] Deleting old gateway...")
                client.delete_asset(asset_id, old_gateway_etag)
                print(f"  ✅ Recreation for '{asset_name}' complete.")
            
            except Exception as e:
                print(f"  ❌ ERROR: An error occurred during gateway recreation for '{asset_name}'.")
                print(f"    Details: {e}")
...
def main():
    ...
    id_map = create_id_map(local_data, server_hierarchy_data)
